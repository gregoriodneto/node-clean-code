...
export class FileController {
    ...
    save: RequestHandler = async (req: Request, res: Response): Promise<void> => {
        ...
        if (!filename || !content) {
            await HistoryService.save(`[/files] Filename e Content são obrigatórios.`);
        ...
        try {
            ...
            await HistoryService.save(`[/files] Salvando mensagem no arquivo ${filename} com o conteúdo: ${content}.`);
            res.status(HttpStatus.CREATED).json({ message: 'Mensagem salva com sucesso.' });
        } catch (error) {
            HistoryService.save(`[/files] Erro ao salvar o arquivo.`);
    ...
    show: RequestHandler = async (req: Request, res: Response): Promise<void> => {
        ...
        if (!filename) {
            await HistoryService.save(`[/file/:filename] Filename é obrigatório no ${filename}`);
        ...
        try {
            ...
            await HistoryService.save(`[/file/:filename] Buscando todas as mensagens salvas no arquivo ${filename}.`);         
            res.status(HttpStatus.OK).json({ message: 'Mensagens carregadas com sucesso.', content });
        } catch (error) {
            await HistoryService.save(`[/file/:filename] Erro ao ler o arquivo ${filename}. ${error}`);
